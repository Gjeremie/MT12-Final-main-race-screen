...
local old15 = math.floor(getValue('gvar3'))
...
if  math.floor(getValue('gvar3')) ~= old15 then -- regarde si action sur changement de valeur avec  potar
 	ret15 =  getTime()
end
old15 =  math.floor(getValue('gvar3')) -- assigne ancienne valeur
 
if getTime() < ret15 + 200  then -- vérif si depuis 2 seconde valeur a plus bougé


  lcd.drawNumber(53, 52, math.floor(getValue('gvar3')/6-40/6+0.5) , MIDSIZE+INVERS) -- valeur  --  si depuis moin de 2 sec alors affiche valeur live du potar
else -- afficher valeur sauvegardé

 		
	
	  if mem[15] == 0 then
 lcd.drawNumber(53, 52, 10 , MIDSIZE) -- valeur 10
 else
lcd.drawNumber(53, 52, math.floor(mem[15]/6-40/6+0.5) , MIDSIZE) -- valeur 
  end
...
for i = 1,10 do
	...
	rebound = getTime()
	  ...
	  if edit == 11 then -- bouton save
	   ...
	   local file = io.open(fich2, "a") -- ouvrir fichier 0-mem.txt en acces ecriture et en preservant son contenu
					...
					mem[15] = math.floor(getValue('gvar3'))  -- valeur  g3 de 40 a 100 etage frein
						if mem[15] == 100 then
						mem[15] = 0
						end
	...
	modif = 0 -- ACTION sauvegarde valeur actuelle et revien etage 0
		...
		if mem[15] == 0 then
			model.setGlobalVariable(2, getFlightMode(), 100) -- applique sauvegarde a valeur   gvar3
			else
			model.setGlobalVariable(2, getFlightMode(), mem[15]) -- applique sauvegarde a valeur   gvar3
			end
