...
lcd.drawText(1, 47, "Telemetry Alert:" , SMLSIZE+INVERS) -- texte fond noir
...
if rot==11 then -------------- VALEUR reglage ventilo   /   output 3
...
lcd.drawText(74, 56, math.floor(model.getOutput(2).max/10) .." %" , SMLSIZE+INVERS) -- texte fond noir
		...
		if tab.max == 0 then
		valeur = 100 -- lire valeur de output
		else 
		valeur = tab.max
		end
		...
		if event == EVT_VIRTUAL_NEXT then -- bouton rotary next 
			valeur = choose(valeur,100,1000,100) -- detection rotary next ou prev
		end
		if event == EVT_VIRTUAL_PREV then -- bouton rotary next 
		 valeur = choose(valeur,100,1000,-100) -- detection rotary next ou prev
		end	
...
elseif (rot >11 and rot<19) then --========================    PAGE 2    ======================  
...
lcd.drawText(1, 47, "Coeff frein Arr:" , SMLSIZE+INVERS) -- texte fond noir
...
lcd.drawText(86, 20, model.getOutput(0).ppmCenter  , SMLSIZE) -- texte
 ...
 if rot==15 then -------------- nb dent pinion moteur /   Logic Switch L08  
...
lcd.drawText(73, 29, model.getLogicalSwitch(7).v2 .. " dents" , SMLSIZE) -- texte
   ...
   if rot==16 then -------------- VALEUR point speed thr   /   Logic Switch L14 
...
lcd.drawText(86, 38, model.getLogicalSwitch(13).v2 .. " %" , SMLSIZE) -- texte
    ...
    if rot==17 then -------------- VALEUR coeff frein arr /   channel 4 copier les ligne de mix   
   
   ------ affiche valeur selectionnne -----
  
   lcd.drawText(81, 47,model.getMix(3, 2).weight .. " %", SMLSIZE+INVERS) -- texte fond noir
	
      
   	if modif == rot then -- si mode modif sur cet item
		  
		   valeur = model.getMix(3, 2).weight -- recup valeur actuelle tps drag brake
	
	
	if event == EVT_VIRTUAL_NEXT then -- bouton rotary next 
			valeur = choose(valeur,50,150,1) -- detection rotary next ou prev
			
			model.deleteMix(3, 2) -- suppr ligne 3 de channel 4
			tab = model.getMix(3, 1) -- copier ligne 2
			tab.weight = valeur -- modif weight de ligne copier
			
		model.insertMix(3, 2, tab) -- copier une ligne de mix et la coller dans ligne 3
   ...
   lcd.drawText(81, 47,model.getMix(3, 2).weight .. " %", SMLSIZE) -- texte fond noir
...
if rot==18 then -------------- VALEUR feeling St  Ultra Fast ou Fast ou Medium ou Slow /   channel 10 copier les ligne de mix   
...
elseif (rot >18 and rot<26) then --========================    PAGE 3    ======================  
...
lcd.drawText(1, 56, "Mode Enfant:" , SMLSIZE+INVERS) -- texte fond noir
 ...
 -- ========================== affiche valeur ========================================
   ...
   if rot==19 then -------------- VALEUR reglage duty cycle  ABS  LS20
...
if rot==20 then -------------- VALEUR reglage hold time ABS curve abs
...
lcd.drawText(lcd.getLastPos()+2, 20, ")", SMLSIZE)  
  ...
  if rot==21 then -------------- VALEUR Tps drag brake /   channel 19 copier les ligne de mix   
  ...
  if rot==22 then -------------- VALEUR agressivité drag brake /   channel 20 copier les ligne de mix   
...
pos = tab.x
 ...
 if rot==23 then -------------- VALEUR fall drag brake  /   curve 25
 ...
 if rot==24 then -------------- ACTIVATION limiteur DR bouton    /   Special Function CF11  / Logical switch  LS44 
...
case(94,46,model.getCustomFunction(10).active) -- case
   ...
   if rot==25 then -------------- VALEUR reglage mode enfant /   output 1 (ch2)
