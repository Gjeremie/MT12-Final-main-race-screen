...
local shared = { }   
shared.screens = {
"/SCRIPTS/TELEMETRY/0-M1S.lua", -- screen 1
  "/SCRIPTS/TELEMETRY/0-M11.lua", -- screen 2
  "/SCRIPTS/TELEMETRY/0-M2.lua", -- screen 3
  "/SCRIPTS/TELEMETRY/0-M3.lua", -- screen 4
  "/SCRIPTS/TELEMETRY/0-M4S.lua", -- screen 5
  "/SCRIPTS/TELEMETRY/0-M4.lua", -- screen 6
  "/SCRIPTS/TELEMETRY/0-M5.lua", -- screen 7
  "/SCRIPTS/TELEMETRY/0-M6.lua", -- screen 8
  "/SCRIPTS/TELEMETRY/0-M7.lua", -- screen 9
  "/SCRIPTS/TELEMETRY/0-M8.lua", -- screen 10
  "/SCRIPTS/TELEMETRY/0-M9.lua", -- screen 11
  "/SCRIPTS/TELEMETRY/0-M12.lua", -- screen 12
  "/SCRIPTS/TELEMETRY/0-M13.lua", -- screen 13
  "/SCRIPTS/TELEMETRY/0-M71.lua", -- screen 14
  "/SCRIPTS/TELEMETRY/0-M72.lua", -- screen 15
  "/SCRIPTS/TELEMETRY/0-M91.lua" --  screen 16
}
...
local diresec = -1 -- annonce tps secteur 1
local minisec = 60000 -- secteur 1 mini
...
shared.lat1 = 24569865  -- latitude secteeur 1 (ligne de départ position sauvegardé sur SD) decalage par rapport a lat 45°
shared.lon1 = -5430172   -- longitude secteur 1 ecalage par rapport a lon 5°

local lat = 0 -- latitude
local lon = 0 -- longitude

local lat1 = 0 -- latitude secteur 1
local lon1 = 0 -- longitude secteur 1
...
setTelemetryValue(0x0025, 0, 0, 10000) -- remettre fake sensor capa a   100 00cm
...
local function data6(x)   -- sauvegarde mise en forme 6 carac
    x = tostring(x)  -- convertir en string
    
    if #x < 6 then
        -- Ajouter des "x" devant pour compléter jusqu'à 6 caractères
        x = string.rep("x", 6 - #x) .. x
    elseif #x > 6 then
        -- Si la chaîne est trop longue, la tronquer à 6 caractères
        x = string.sub(x, 1, 6)
    end
...
local file = io.open(fich1, "r") -- ouvrir fichier 0-timr.txt en acces lecture
		...
		local curs = io.seek(file, 10+40+90*(preset-1)) --  positionner curseur dans fichier pour lire lat GPS origine
		...
		local curs = io.seek(file, 10+50+90*(preset-1)) --  positionner curseur dans fichier pour lire lon GPS origine
		...
		local curs = io.seek(file, 10+60+90*(preset-1)) --  positionner curseur dans fichier pour lire lat GPS origine
		shared.lat1 = io.read (file, 10) -- lire 10 carac de  dans fichier et asssigner
		
		shared.lat1 = tostring(shared.lat1)  -- convertir en strings
		shared.lat1 = string.gsub(shared.lat1,"x","")  -- enlever les xx 
			if (shared.lat1=="") then -- variable non assigné
				shared.lat1 = "24569865"  -- mettre variable a a
			end
		shared.lat1 = tonumber(shared.lat1) -- convertir en nombre ASSIGNER LATITUDE secteur 1
		
		
		
		local curs = io.seek(file, 10+70+90*(preset-1)) --  positionner curseur dans fichier pour lire lon GPS origine
		shared.lon1 = io.read (file, 10) -- lire 10 carac de  dans fichier et asssigner
		
		shared.lon1 = tostring(shared.lon1)  -- convertir en strings
		shared.lon1 = string.gsub(shared.lon1,"x","")  -- enlever les xx 
			if (shared.lon1=="") then -- variable non assigné
				shared.lon1 = "-5430172"  -- mettre variable a a
			end
		shared.lon1 = tonumber(shared.lon1) -- convertir en nombre ASSIGNER LONGITUDE secteur 1
...
sc = getValue('input14') -- valeur switch sc entree 14
	...
	if (tim3<2 and ses ==0) then -- variable timer 3 a 0 (ce qui veut dire debut de SESSION 
		 ...
		 minisec = 60000 -- secteur 1 mini
		 mina = 0 -- 
		diresec = -1 -- reset secteur 1
...
dire = lap
diresec = 0 -- activer detection secteur 1
mina= 1
lapold = getTime()

		if (shared.num>-1) then -- si pas premier tour
...
if (  ( diresec == 0 ) and ( getValue('ls38') > 0 and shared.num>-1) )   then -- si detection distance gps par rapport au point secteur 1 (ls38) est ON   AND  si passage ligne tour effectué

diresec = getTime() - lapold -- assigner tps secteur 1 depuis ligne depart tour precedent

	if diresec < minisec then -- si on est en avance sur le tps secteur mini 
		if shared.num>1 then -- si pas premier ni 2eme tour
		playFile("/SCRIPTS/TELEMETRY/sect.wav") -- annonce secteur
		playFile("/SCRIPTS/TELEMETRY/moins.wav") -- annonce moins
		playNumber(math.floor(-(diresec-minisec)/10),0, PREC1)  -- lire nombre avec 1 decimale si 123 il lit 12.3
		end
	minisec = diresec -- assigner le secteur mini
	else -- si on est en retard sur le tps secteur mini 
		if shared.num>1 then -- si pas premier ni 2eme tour
		playFile("/SCRIPTS/TELEMETRY/sect.wav") -- annonce secteur
		playFile("/SCRIPTS/TELEMETRY/plus.wav") -- annonce plus
		playNumber(math.floor((diresec-minisec)/10),0, PREC1)  -- lire nombre avec 1 decimale si 123 il lit 12.3
		end
	end

diresec = -1 -- desactiver detection secteur jusqua prochain passage ligne depart
end


-- faire bipper au moment meilleur tour et annonce tour avec retard
...
-- capacité conso :
	...
	-- tension lipo live :
		if voltON == true then
			...
			if tonumber(volt) < 490 then -- si tension inf a 4.9v sauvegarder, sinon non car on est en phase allumage ou autre plantage watchdog
...
-- GPS
		if lat ~= 0 then -- Si GPS a un fix
			local curs = io.seek(file, 186) --  positionner curseur dans fichier log
			-- latitude a 10e-5  ( precison de 1.11m ) :
			local ecri = io.write (file, data6(math.floor(lat/100)) ) -- ecri 6 carac dans fichier log
			local curs = io.seek(file, 192) --  positionner curseur dans fichier log
			-- longitude  a 10e-5  ( precison de 0.7 m ) :
			local ecri = io.write (file, data6(math.floor(lon/100)) ) -- ecri 6 carac dans fichier log
		end
...
setTelemetryValue(0x0024, 0, 0, 5000 ) -- maintien capteur live-- affecte 5000 pour faire clignoter telco
	...
	if Rvolt > 1000 then -- verif tension transmise > 0
	setTelemetryValue(0x0024, 0, 0, Rvolt ) -- maintien capteur live
...
-- calcul distance avec position GPS sauvegardé (position origine)	
		...
		-- pour le secteur 1 soustraction pour distance , on fera la racine ensuite
	   lon1 = (lon - shared.lon1)  * 67 // 100 -- coeff car difference entre latitude et longitude degré sur terre
	   lat1 = (lat - shared.lat1)
	...
	lat1 = math.floor((( math.max(math.abs(lat1), math.abs(lon)) + math.min(math.abs(lat1), math.abs(lon1)) ) //2 )*111/100 )-- approximation de la distance sans utiliser pythagore et conversion en cm pour secteur 1
...
setTelemetryValue(0x0023, 0, 0, lat ) -- assigne distance en cm dans sensor lapD pour origine GPS
	
setTelemetryValue(0x0025, 0, 0, lat1 ) -- assigne distance en cm dans sensor secD pour secteur 1	
