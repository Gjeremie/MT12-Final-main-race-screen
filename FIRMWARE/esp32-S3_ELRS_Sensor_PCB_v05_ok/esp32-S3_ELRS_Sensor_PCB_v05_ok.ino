...
RunningMedian MOYtension = RunningMedian(30); // mediane glissante de 30 - tension    
...
void hwtCallback() {
  ...
  courant = HWTelemetry.getCurrent();
...
lastTelemetryTime = millis();       // temps du dernier calcul de fréquence
 ...
 // Assigner  courant 
	MOYcourant.add(courant*1000); // ajoute derniere valeur relevé courant a mediane en mA
...
MOYtension.add(6000); // ajoute derniere valeur relevé tension a mediane en mv,
MOYcourant.add(courant*1000); // ajoute derniere valeur relevé courant a mediane en mA
		...
		// ********* envoie packet RX BATTERY
				...
				if (millis()<30000) { // a faire au debut pendant 30 sec
					if ((millis()-pulsedebut)>5000) { // a faire tous les 5000 ms et pendant 200ms
					  sendRxBattery(6000, MOYcourant.getAverage()/10.0, MOYtemperature1.getMedian(), 1);   // envoie valeur avec tension fake a 6V 
